...
import local.library.ws.soap.generic.client.util.AnnotationFactory;
...
public class WsdlDocument {
    ...
    private Map<String, XSElementDecl> schemaElements = new HashMap<>();

    /**
     * Initializes libraries needed for accessing the wsdl and the inner xml
     * schema
     *
     * @param url Reference pointing to the wsdl
     * @throws IOException if there is a problem reading the wsdl
     */
    public WsdlDocument(URL url) throws IOException {
        try {
            ...
            wsdlInterface = WsdlInterfaceFactory.importWsdl(project, wsdlUrl.toString(), false)[0];
    ...
    private void initXsom() throws IOException {
        try {
            SchemaImpl schema = (SchemaImpl) wsdlObject.getTypes().getExtensibilityElements().get(0);
            ...
            if (parser.getResult() == null)
                throw new IOException("Could not initialize XML Schema");

            for (XSSchema xsSchema : parser.getResult().getSchemas()) {
                schemaObject = xsSchema;
                addSchemaElements();
            }

            // set schema on target namespace for MultiValued check
    ...
    private void addSchemaElements() {
        for (Map.Entry<String, XSComplexType> entry : schemaObject.getComplexTypes().entrySet()) {
                ...
                if (term.isModelGroup()) {
                    ...
                    for (XSParticle p : particles) {
                        ...
                        if (pterm.isElementDecl()) {
                            ...
                            XSParticle[] tparticles = pterm.asModelGroup().getChildren();
                            for (XSParticle tp : tparticles) {
                                XSTerm tpterm = tp.getTerm();
                                if (tpterm.isElementDecl()) {
                                    XSElementDecl e = tpterm.asElementDecl();
                                    schemaElements.put(e.getName(), e);
                                }
                            }
                        }
     ...
     *
     * @param inputName The element name
     * @return
     */
     ...
     * @param operationName
     * @return
     * @throws IOException if there is no such operation
     */
    public String getOperationDocumentation(String operationName) throws IOException {
    ...
    private Operation getOperation(String operationName) throws IOException {
        ...
        PortType p = (PortType) wsdlObject.getPortTypes().values().iterator().next();
     ...
     *
     * @param operationName Operation in which the input is located
     * @return
     * @throws IOException if there is no such operation
     */
    public String getInputDocumentationsFromWsdl(String operationName) throws IOException {
        Operation op = getOperation(operationName);
