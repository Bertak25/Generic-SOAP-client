...
public class SoapOperation {
    ...
    private Logger logger = LogManager.getLogger(SoapOperation.class);
    private static final int SOAP_REQUEST = 0;
    private static final int SOAP_RESPONSE = 1;

    private String name;
    private Map<String, SoapInput> inputs = new HashMap<>();
    private Map<String, SoapOutput> outputs = new HashMap<>();
     ...
     * @param wsdlDoc contains info about the operation
     * @throws IOException
     */
    ...
    private void parseSoap(Element current, int soapType) throws IOException {
        if (current.getChildren().size() == 0 && !current.getText().equals("")) {
            if (soapType == SOAP_REQUEST) {
                SoapInput input = new SoapInput(current.getName(), wsdlDoc, this.name);
                inputs.put(current.getName(), input);
            } else if (soapType == SOAP_RESPONSE) {
                SoapOutput output = new SoapOutput(current.getName(), soapResponse);
    ...
    public Map<String, SoapOutput> execute(String user, String pass, boolean parse) throws IOException {
        try {
            ...
            throw new IOException("Problem while parsing the SOAP request message.", e);
        }
    ...
    public List<SoapAttachment> getReceivedAttachments() {
        List<SoapAttachment> list = new ArrayList<>();

        Attachment[] atts = wsdlResponse.getAttachments();
        for (Attachment att : atts) {
            try {
                SoapAttachment temp = new SoapAttachment(att.getInputStream(), att.getContentType());
                list.add(temp);
            } catch (Exception e) {
